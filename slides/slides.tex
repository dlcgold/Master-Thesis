...
\begin{frame}{Il pangenoma}
  ...
  % \end{block}
  \only<1>{
    \begin{block}{Un cambio di paradigma}
      \begin{itemize}
        \item singolo genoma $\implies$ insieme di genomi
        \item studio delle varianti geniche tra genomi di diversi
        individui 
      \end{itemize}
    \end{block}
...
\begin{frame}{Trasformata di Burrows--Wheeler posizionale}
  \only<1>{
    ...
    \end{block}
    \begin{block}{}
      Aplotipi simili, riordinati consecutivamente alla colonna $k$, è molto
      probabile presentino il medesimo carattere in colonna $k+1$.
    \end{block}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[scale = 0.8]{img/matrix2.pdf}
    \end{figure}
  }
  % \only<3>{
  %   \begin{figure}[H]
...
\begin{frame}{Scopo della tesi}
  ...
  \begin{block}{}
    ...
    \begin{itemize}
      ...
      \item spazio: $\mathcal{O}(NM)\implies$ \underline{\textbf{13NM byte}}
    \end{itemize}
  \end{block}
  % \begin{block}{}
...
\subsection{Strutture dati composte per la RLPBWT}
\begin{frame}{Sottostrutture per la RLPBWT}
  \begin{block}{}
    \small{Sottostrutture per ottenere strutture dati composte per la $\RLPBWT$}
  \end{block}
  \vspace{-2mm}
  % \begin{block}{Sottostrutture per ottenere strutture dati composte per
  %     la $\RLPBWT$} 
  %   \begin{itemize}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{img/ds2.pdf}
  \end{figure}
\end{frame}
...
\begin{frame}{Matching statistics}
 ...
 \begin{figure}[H]
    ...
    \includegraphics[scale = 0.38, valign=t]{img/mini_ms.pdf}
  \end{figure}
  \vspace{-0.5cm}
  \begin{table}[H]
    \scriptsize
...
\begin{frame}{Struttura per le funzioni $\varphi$ e $\varphi^{-1}$}
  \begin{figure}[H]
    \centering
    \includegraphics[scale = 0.8, valign=t]{img/phi.pdf}
    \includegraphics[scale = 0.55, valign=t]{img/mini_phi.pdf}
  \end{figure}
...
\begin{frame}{Dati di input}
  ...
  \begin{block}{}
    \begin{itemize}
      ...
      \item numero costante di aplotipi: 5908
...
\begin{frame}{}
  ...
  \end{block}
  \begin{block}{}
    Il lavoro verrà presentato l'11 Novembre al \textit{17th Workshop on
      Compression, Text, and Algorithms (WCTA)} 
  \end{block}
  \begin{alertblock}{}
    \begin{center}
  ...
  \begin{figure}[H]
    ...
    \includegraphics[width = 0.16\textwidth]{img/bias_gh.png}$\quad\quad$
