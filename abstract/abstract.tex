...
in a large collection of individual genomes. This new reference, which
was firstly introduced in 2005 for dealing with the comparison of genes in
bacterial species, is called a \emph{pangenome}.  
...
thousand individuals, in the next few years, also thanks to the latest
developments in sequencing technologies, (\textbf{Next Generation Sequencing
...
% In this context, my PhD is going to be focused on the development of new
% algorithms in 
...
A \textit{bitvector}, of the most important \textit{succint data
  structures}, is an array on $n$ bits that allows two particular
...
1 present in the \textit{bitvector}. Formally, given a bitvector $B$, such
...
\begin{enumerate}[leftmargin=.2in]
  ...
  haplotype sequences as 
  \textit{pangenome graphs}, in a compact way via the \textbf{Graph BWT
    (\textit{GBWT})}, even if it is more a \textit{multi-string BWT} than a
  classical graph. A first 
  ...
  \textit{BIAS} laboratory's topics of interest. So far there has been talked of
...
It is therefore my interest to contribute to the development of such open-source
libraries. Another point of interest is the 
analysis and the development of efficient algorithms based on parallel computing
(also on \textit{GPU}), that are increasingly in use in both
\textit{bioinformatics} and \textit{computational systems biology}.
