...
Ad esempio, un pannello $4.908 \times 1.055.454$ richiede per essere
rappresentato in forma non compressa circa 4,8GB di memoria, mentre tramite SLP
richiede 
solamente in 44,8MB (garantendo random access). Per riferimento, comprimere il
pannello con una tecnica 
standard (GZip) richiede 96.1MB. Oltre all'uso dell'SLP (componente
...
pannello del cromosoma 22, di dimensioni $4.908 \times 1.055.454$, che è stato
...
la PBWT si ottiene il calcolo degli SMEM in 143,94s
mentre con la RLPBWT in 250,25s.
...
picchi di memoria prossimi ai 62,9GB di memoria, confermando le stime. Invece,
per la RLPBWT, con la struttura dati composta presa in esame, il picco
registrato è stato di appena 0,78GB, 
cioè si è ottenuta una riduzione del 98,75\% di memoria rispetto alla soluzione
...
termini di tempo (17,27s) che di memoria utilizzata (23.14MB) durante
