...
considerare anche le \textit{variazioni} o le \textit{varianti geniche}. 
Questo nuovo concetto è stato nominato per la prima 
volta, nel 2005, da Tettelin \cite{tettelin} con il termine
\textbf{pangenoma}.\\ 
Grazie ai risultati ottenuti in \textit{pangenomica} si sono permessi
miglioramenti sia nel campo della \textit{biologia} che, ad esempio,
della \textit{medicina personalizzata}, soprattutto grazie ai
\textbf{genome-wide association studies (\textit{GWAS})} \cite{gwas}
\cite{impute5}. Infatti, in tali studi, si 
...
ad esempio prendendo come reference l'\textit{homo sapiens reference
genome GRCh38.p14}, è composto da circa 
3.1 miliardi di basi, comportanti circa 59,000 geni \cite{tutorial}. Inoltre, i
risultati del 
...
che tale è destinata ad aumentare nel prossimo futuro, grazie al miglioramento
delle tecnologie di sequenziamento (\textbf{Next Generation Sequencing
  (\textit{NGS})} e \textbf{Third-Generation Sequencing}).\\ 
...
tramite il \textbf{grafo del pangenoma} e una tramite un \textbf{pannello di
  aplotipi} \cite{pancon}. In entrambi i casi sono necessarie metodologie
efficienti sia per 
