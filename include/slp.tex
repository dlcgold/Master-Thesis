...
\textit{SLP} e lungo $n$, sia in tempo:
\begin{equation}
  \label{eq:slptime}
...
Si stima che il calcolo di una \textit{LCE query} di lunghezza $l$ sia
effettuabile, per l'\textit{SLP} di un testo $T$ lungo $n$, in tempo: 
\begin{equation}
  \label{eq:lcetime}
  \mathcal{O}\left(\log n+l\right)
\end{equation}
Che di fatto è proporzionale a:
\begin{equation}
  \label{eq:lcetime2}
  \mathcal{O}\left(\log n\right)
\end{equation}
