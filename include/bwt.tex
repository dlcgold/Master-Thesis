...
In questo progetto, non solo si è sostituito l'uso delle threshold con
...
delle teste delle run avviene tramite l'uso di bitvector, che tengono traccia le
teste di run per ogni simbolo dell'alfabeto. Infatti,
...
\begin{equation}
  ...
  B_{\sigma}[i]=
  \begin{cases}
    1&\mbox{se } \BWT_T[i]=\sigma\land \BWT_T[i]\neq\BWT_T[i-1]\\
    0&\mbox{altrimenti}
  \end{cases}
\end{equation}
...
\end{algorithm}
\begin{esempio}
  Dati il testo $T=\mbox{mississippi\$}$ e il pattern $P=\mbox{miss}$,
  si vuole studiare il calcolo dell'array delle matching statistics sia con
  MONI che con PHONI. Si assume che $|T|=n$ e $|P|=m$.
  Avendo, in aggiunta in ultima colonna, i cinque bitvector relativi alle
