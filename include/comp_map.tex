...
al mapping tra una colonna e la sua successiva nella matrice $\PBWT$. In altri
termini, bisogna 
studiare come effettuare il forward-step/mapping nella $\RLPBWT$. Bisogna
memorizzare, per 
...
memorizzare gli indici delle teste di run. Ovviamente, quest'informazione non è
...
pari al numero di run in colonna $k$. Si memorizza un indice $i\in\{0,M-1\}$
sse: 
\begin{equation}
  \label{eq:naive1}
  y_{i-1}^k[k]\neq y_i^k[k]
...
per il mapping fossero tutte necessarie, ovvero se, data la colonna $k$
nella matrice $\PBWT$, fossero necessari interamente $c[k]$, $u_k$ e $v_k$. In
merito al valore $c[k]$,  si è deciso che si potesse calcolarlo 
...
per ottenerne una rappresentazione che implicasse memorizzare un solo valore per
ogni 
...
permettessero di effettuare comunque il 
...
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
Tale operazione è eseguibile in tempo costante e lo pseudocodice relativo a
...
ogni colonna $k$:
\begin{itemize}
  \item $start_k$, ovvero il booleano atto a sapere il simbolo della prima
  run
...
\begin{esempio}
  ...
  \end{itemize}
  Si conferma, inoltre, quanto indicato nell'equazione \ref{eq:bv5}, avendo:
  \[\rank_{h_5}(|h_5|-1)+1=5 =
...
Bisogna, in primis, spiegare come, dato un indice di riga
$i\in\{0,\ldots,M-1\}$ e una 
colonna $k$, computare $u'_k[i]$ e $v'_k[i]$, ovvero come se si stesse usando la
...
\begin{esempio}
  ...
  \[offset = i-beg=6-4=2\]
  Quindi ci si trova nel terzo caso e, nel dettaglio, in una run di
...
\begin{equation}
  \label{eq:mapintmem2}
  \approx N\left(\frac{\rho\left(2+\log\frac{M}{\rho}\right)}{4}+29\right)\mbox{ byte} 
\end{equation}
\dc{Ricontrollare conto}
