...
\begin{itemize}
  ...
  $MS$ 
\end{itemize}
Per scelta si ha che la riga da cui parte l'iterazione nella prima colonna
corrisponde all'ultima ma tale scelta è assolutamente arbitraria. In modo
analogo si procede qualora si abbia una colonna $k$ di un solo carattere, non
corrispondente a quello della query. In tal caso si sceglie l'ultima riga della
colonna successiva, da cui riprende il calcolo 
dell'array delle matching statistics, dopo aver memorizzato $MS[k].row=M$
(essendo un valore sentinella non esistendo la riga $M$) e $MS.len[k]=0$.\\
